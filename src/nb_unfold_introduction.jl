...
md"""
First we will need a ground truth (aka response kernel), which will represent the ERP we want to later estimate. For our simulation, we will use two different two different kernels which will be generated by two different functions. In an EEG experiment these could be two different experimental stimuli/ conditions or simply a stimulus and a response.
"""
...
md"""
Or show how our kernels actually map onto these onsets (top), and how the combined continous signal looks like (bottom):
"""
...
!!! tip \"Epoching in Matlab\" 
	In Matlab you can either use EEGLABs native epoching function or uf_epoch() for trhis.
"""
...
md"""
In practice Unfold does all the work for us. And the only thing we have to change compared to our Mass-Univariate model from before is that we have to give Unfold the continous data (instead of the epoched one) and define a set of basisfunctions alongside our model formula.
"""

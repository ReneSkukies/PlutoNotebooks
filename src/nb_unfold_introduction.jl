...
md"""
First, we will need a ground truth (aka response kernel), which will represent the ERP we want to later estimate. For our simulation, we will use two different kernels which will be generated by two different functions. In an EEG experiment, these could be two different experimental stimuli/ conditions or simply a stimulus and a response.
"""

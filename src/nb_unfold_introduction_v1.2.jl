...
let 
	md"""Change the value of b $(@bind b PlutoUI.Slider(4:0.5:6, default=4, show_value=true))"""
end
...
begin
	kernelA = Plots.plot(erp_A, xlims=(-2, 10), ylims=(-5,5), legend=false, linecolor=:orange, size=(600,200))
...
let
	md"""Change the value of d $(@bind d PlutoUI.Slider(3:0.5:6, default=3, show_value=true))"""
end
...
begin
	Plots.plot(erp_B, xlims=(-2, 10), ylims=(-5,5), linecolor=:deepskyblue, legend=false, size=(600,200))
	vline!([0], linestyle=:dash, linecolor=:black)
end
...
begin	
	noise_slider = md"""Change noise: σ = $(@bind σ PlutoUI.Slider(0:0.2:2, default=0, 
		show_value=true))"""
end
...
begin
	window_slider = md"""Change window size τ = (-1.0, $(@bind τ  PlutoUI.Slider(-1:0.1:10,default=7,show_value=true)))"""
end
...
begin	
	noise1_slider = md"""Change noise: σ\_1 = $(@bind σ₁ PlutoUI.Slider(1.:0.01:6, default=1.0, show_value=true))"""
end

# ╔═╡ 53bbb4a7-9052-417a-b8f3-3230608eb85c
begin	
	mean_slider = md"""Change mean:μ\_1 = $(@bind μ PlutoUI.Slider(0:0.01:5, default=0, 
		show_value=true))"""
end
...
let
	p1 = Plots.plot(range[1:801], data_noise[1:801], xlims=(-10, 70), ylims=(-5,5), legend=false,  linecolor=:green, size=(600, 200))
	vline!([0], linestyle=:dash, linecolor=:black)
end

# ╔═╡ c568b352-374b-48ea-9cd3-99f7c1dbb88d
begin
	p1 = Plots.plot(-10:0.1:70, eeg, xlims=(-10, 70), ylims=(-5,5), legend=false,  linecolor=:green)
	vline!([0], linestyle=:dash, linecolor=:black)
	p2 = Plots.plot(-10:0.1:70,eeg_A, xlims=(-10, 70), ylims=(-5,5), legend=false, linecolor=:orange)
	Plots.plot!(-10:0.1:70,eeg_B, xlims=(-10, 70), ylims=(-5,5), legend=false, linecolor=:deepskyblue)
	vline!([0], linestyle=:dash, linecolor=:black)
	vline!(event_onsets_A,linecolor=:orange,linestyle=:dash)
	vline!(event_onsets_B,linecolor=:deepskyblue,linestyle=:dash)
	Plots.plot(p2, p1,layout=@layout[a;b])
...
begin
	...
	Plots.plot(condA_massU.time, condA_massU.estimate, ylims=(-5,5), linecolor=:orange, 
		label="conditionA", legend=:outerbottom)

	# condition B
	condB_massU = filter(row->row.coefname=="conditionB", results_massU)
	pₒ = Plots.plot!(condB_massU.time, condB_massU.estimate, ylims=(-5,7), linecolor=:deepskyblue, 
		label="conditionB")
...
begin
	Plots.plot(range[1:801], data_noise[1:801], xlims=(-10, 70), ylims=(-5,5), legend=false,  linecolor=:green, size=(600, 200))
	vline!([0], linestyle=:dash, linecolor=:black)
end
...
begin
	...
	# condition B
	Plots.plot!(condB_massU.time, condB_massU.estimate, ylims=(-5,7), linecolor=:deepskyblue, 
		label="conditionB")
...
begin
	...
	Plots.plot(condA.time, [condA.estimate condA_massU.estimate], layout = (2,1), ylims=(-5,5), linecolor=:orange, 
	...
	Plots.plot!(condB.time, [condB.estimate condB_massU.estimate], ylims=(-5,5), linecolor=:deepskyblue, 
		label="conditionB")
	Plots.plot!(size=(700,800))
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "41df48b20f0f0ead25d3f64135817c3eedb55fe6"
...
[[deps.RobustModels]]
deps = ["Distributions", "Future", "GLM", "IterativeSolvers", "LinearAlgebra", "Missings", "Printf", "QuadGK", "Random", "Roots", "SparseArrays", "Statistics", "StatsAPI", "StatsBase", "StatsModels", "Tables", "Tulip"]
git-tree-sha1 = "51dd9190eb612e955d91feba783d2eddd25e00d4"
...
[[deps.Unfold]]
deps = ["BSplineKit", "CUDA", "DSP", "DataFrames", "Distributions", "DocStringExtensions", "Effects", "FileIO", "GLM", "IterativeSolvers", "JLD2", "Krylov", "LinearAlgebra", "Logging", "MLBase", "Missings", "MixedModels", "ProgressMeter", "Random", "RobustModels", "SparseArrays", "StaticArrays", "Statistics", "StatsBase", "StatsFuns", "StatsModels", "Tables", "Test", "TimerOutputs"]
